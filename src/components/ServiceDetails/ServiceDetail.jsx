...
const ServiceDetail = ({ service }) => {
  ...
  const handleGetQuote = (id) => {
    alert(id);
  };
  return (
    <div className="">
      <div className="relative">
        <img className="h-[600px] w-full" src={img_url} alt={event_name} />
        <div className="bg-gray-600 bg-opacity-80 w-full p-6 absolute bottom-0">
          <button
            className="p-2 rounded-lg text-white text-3xl  hover:cursor-default font-semibold opacity-100"
          >
            Starting From ${price}
          </button>
          <br/>
          <button
            onClick={() => handleGetQuote(event_id)}
